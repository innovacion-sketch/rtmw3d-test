...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Indice de camara para --input webcam (0=primera).')
    parser.add_argument(
        '--max-frames',
        type=int,
        default=0,
...
def main():
    ...
    if input_type == 'image':
        ...
        if not fps or fps <= 0:
            fps = 30  # webcams suelen reportar 0; valor para el VideoWriter
        ...
        while video.isOpened():
            ...
            if now - fps_win_t0 >= 1.0:
                print(f'[FPS] {fps_win_frames / (now - fps_win_t0):.2f} '
        ...
        if frame_idx > 0 and infer_t_total > 0:
            print(f'[FPS] promedio inferencia: '
                  f'{frame_idx / infer_t_total:.2f} FPS '
                  f'({frame_idx} frames, {infer_t_total:.1f}s)', flush=True)
